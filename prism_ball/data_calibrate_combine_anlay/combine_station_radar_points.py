...
'''
def remove_logs_existd(folder=r'D:\pythonProject\test\pythonProject\smart_eye2_1\prism_ball\data_calibrate_combine_anlay\files_combined' ):
    """删除目标文件夹中的所有文件（保留文件夹本身）"""
    for item in os.listdir(folder):
        item_path = os.path.join(folder, item)
        if os.path.isfile(item_path):
            os.remove(item_path)
def remove_logs_combined():
    src_dir = r'D:\pythonProject\test\pythonProject\smart_eye2_1\prism_ball\data_calibrate_combine_anlay'  # 例如 r'C:\my_project'
    dst_dir = r'D:\pythonProject\test\pythonProject\smart_eye2_1\prism_ball\data_calibrate_combine_anlay\files_combined'  # 例如 r'C:\backup'
