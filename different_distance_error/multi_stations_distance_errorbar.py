...
####################设站坐标
# total_location1=np.array([-0.6933,0.7580,-0.2047])	#0号楼
# total_location2=np.array([-0.0381,1.4164,-0.1631])  #创新大厦2楼平台
# total_location3=np.array([1.2798,0.8476,-0.0776])	#1楼
...
TS_file1 =['0号楼去掉设站点']
TS_file2 =['创新大厦去掉设站点']
#TS_file3 =['2025003第一次后方交会']   #['1号楼去掉设站点']
TS_file3 =['1号楼去掉设站点1']
PC_file='AllGeoPoints*.txt'
cloude_path1=r"F:\2.1SE-T32\2.1采集数据\20250115精度折线\5站折线图\0号楼/"
cloude_path2=r"F:\2.1SE-T32\2.1采集数据\20250115精度折线\5站折线图\创新大厦2楼平台/"
cloude_path3=r"F:\2.1SE-T32\2.1采集数据\20250115精度折线\5站折线图\1楼/"
...
for fold in folds:
		...
		for TS_file in TS_files:                              #读取截取的点云（全站仪打点）
			...
			for i in range(len(points_TS)):
				...
				try:
					...
					#print(v)
					points_TS[i, 4]=v
					scan_dist = np.linalg.norm(points_TS[i, 0:3])
					#scan_dist = np.linalg.norm(points_TS[i, 0:3], axis=1)
	...
	cleaned_lst1 = np.round(cleaned_lst1, decimals=4)
	cleaned_lst=cleaned_lst1.tolist()

	cleaned_lst_mean1 = np.array(cleaned_lst_mean)
	cleaned_lst_mean1 = np.round(cleaned_lst_mean1, decimals=4)
	...
	plt.plot(lable1[0:lenth[0]],cleaned_lst[0:lenth[0]],label='0号楼')
	plt.plot(lable1[lenth[0]:lenth[1]],cleaned_lst[lenth[0]:lenth[1]],label='创新大厦')
	plt.plot(lable1[lenth[1]:lenth[2]],cleaned_lst[lenth[1]:lenth[2]],label='1号楼')
	plt.title("测试误差m")
	plt.xlabel("测试距离m")
	plt.ylabel("测试误差m")
	plt.legend()
	...
	for i in range(len(lable1)):  # 使用 range(len(x)) 确保不会超出索引范围
		plt.text(lable1[i], cleaned_lst[i], str(cleaned_lst[i]), ha='right', va='bottom')
...
for i in range(len(lable1)):  #range(len(lable1)) 使用 range(len(x)) 确保不会超出索引范围
	if i <lenth[0]:
		axs[0].text(lable1[i]+1, cleaned_lst_mean[i],f'{cleaned_lst_mean[i]:.4f} ± {cleaned_lst[i]:.4f}', ha='right', va='bottom')
	if lenth[0]<= i <lenth[1]:

		axs[1].text(lable1[i]+1, cleaned_lst_mean[i], f'{cleaned_lst_mean[i]:.4f} ± {cleaned_lst[i]:.4f}', ha='right',va='bottom')
	if lenth[1] <= i <lenth[2]:

		axs[2].text(lable1[i]+1, cleaned_lst_mean[i], f'{cleaned_lst_mean[i]:.4f} ± {cleaned_lst[i]:.4f}', ha='right',va='bottom')
